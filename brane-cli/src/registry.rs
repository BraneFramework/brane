...
pub fn get_graphql_endpoint() -> Result<String, RegistryError> {
    Ok(format!("http://{}/graphql", InstanceInfo::from_active_path().map_err(|err| RegistryError::InstanceInfoError { err })?.api))
}
...
pub fn get_packages_endpoint() -> Result<String, RegistryError> {
    Ok(format!("http://{}/packages", InstanceInfo::from_active_path().map_err(|err| RegistryError::InstanceInfoError { err })?.api))
}
...
pub fn get_data_endpoint() -> Result<String, RegistryError> {
    Ok(format!("http://{}/data", InstanceInfo::from_active_path().map_err(|err| RegistryError::InstanceInfoError { err })?.api))
}
