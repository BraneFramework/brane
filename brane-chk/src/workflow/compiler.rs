...
use brane_shr::errors::confidentiality::{BinaryError, ConfidentialityKind};
use clap::Parser;
...
#[inline]
fn input_to_workflow(path: &str, lang: InputLanguage) -> anyhow::Result<Workflow> {
    // Read the input file
    ...
    match lang {
        InputLanguage::Wir => {
            ...
            let wir: Wir = serde_json::from_str(&input).with_context(|| {
            ...
            compile(wir).context("Failed to compile input Brane WIR '{wir_id}' to a workflow")
        },
...
#[inline]
fn workflow_to_output(path: &str, lang: OutputLanguage, workflow: Workflow) -> anyhow::Result<()> {
    ...
    let output: String = match lang {
        OutputLanguage::Workflow => {
            ...
            serde_json::to_string_pretty(&workflow).with_context(|| format!("Failed to serialize given workflow '{}'", workflow.id))?
        },
    ...
    if path == "-" {
        ...
        std::io::stdout().write_all(output.as_bytes()).context("Failed to write to stdout")?;
    } else {
        debug!("Writing result to output file '{path}'...");
        fs::write(path, output).with_context(|| format!("Failed to write to output file '{path}'"))?;
    }
...
fn run(args: Arguments) -> Result<(), BinaryError> {
    ...
    let workflow: Workflow = input_to_workflow(&args.input, args.input_lang).map_err(|source| BinaryError {
        confidentiality: ConfidentialityKind::Confidential,
        msg: "Could not convert input to workflow".into(),
        identifier: 4,
    ...
    workflow_to_output(&args.output, args.output_lang, workflow)
        .map_err(|source| BinaryError::new("Could not convert workflow to output".into(), Some(source.into_boxed_dyn_error()), ExitCode::FAILURE))?;
