...
pub enum Error {
    ...
    #[error("User not specified in given workflow")]
    MissingUser,
    ...
    #[error("Parallel edge at {pc}'s merge pointer {merge} is out-of-bounds")]
    ...
    #[error("Parallel edge at {pc}'s merge edge (at {merge}) was not an Edge::Join, but an Edge::{got}")]
